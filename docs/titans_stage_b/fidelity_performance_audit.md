...
Selected default: `exact_accelerated+sdpa`. READY requires every Stage B exit criterion, a reproducible selected default, causal/state contracts, target-hardware performance, and retained synthetic mechanism advantage.

## Stage B exit criteria

| Criterion | Result | Evidence |
| --- | --- | --- |
| Reference backend and all immutable Stage A gates pass | PASS | Stage A gates=True; B1 reference parity=True; final suite=94 passed/7 warnings |
...
| MacBook and named A100 measurements are reproducible | PASS | MacBook debug/nimble artifacts and installed rerun command exist; named A100 pilot measured and strictly verified |
...
| --- | --- | --- | --- | --- |
| reference | FP64 oracle / FP32 reference | `defaults: memory=reference, attention=multihead_attention, gates=token_wise, activation=float32` | PASS: immutable Stage A and B7 future-perturbation tests | False |
...
| exact_accelerated+sdpa | tensor-exact FP32 combined execution backend | `memory=exact_accelerated; attention=sdpa; activation=float32` | PASS: B3 exact recurrence plus B6 exact-mask and future-prefix evidence | True |
| flash | unavailable: forced exact-mask A100 probe rejected the kernel | `attention=flash only after probe_and_enable_flash succeeds` | Not selectable; the exact mask was never weakened | False |
...
- Hardware/dtype/precision: Intel MacBook CPU plus named NVIDIA A100-SXM4-40GB; FP64 oracle and FP32 execution
- Geometry/timing protocol: B1 1x d=8: 326.69 tok/s, 1 warmup/3 reps; B7 nimble 4x d=256: 53.11 tok/s; A100 two-segment training 60.65 tok/s, 3 reps
- Evidence artifact: `artifacts/titans_stage_b/b1_reference_macbook.json; artifacts/titans_stage_b/a100/a100_training_step_matrix.json`
...
- Limitations: A100 evidence is a small synthetic two-segment pilot, not a 2,048-token capacity result.
...
- Hardware/dtype/precision: Intel MacBook CPU FP32 plus named NVIDIA A100-SXM4-40GB FP32
- Geometry/timing protocol: B3 debug speedup 1.232x; nimble 0.821x; 1 warmup/1 rep; A100 B3 1.013x and two-segment training 63.54 tok/s, 3 reps
- Evidence artifact: `artifacts/titans_stage_b/b3_exact_acceleration_matrix.json; artifacts/titans_stage_b/a100/b3_exact_acceleration_matrix.json; artifacts/titans_stage_b/a100/a100_training_step_matrix.json`
...
- Limitations: Only modestly faster alone on the A100 pilot; the combined exact-memory plus SDPA configuration won the default gate.
...
- Hardware/dtype/precision: CPU FP64/FP32/BF16 plus A100 FP32/BF16/FP16; published output and memory state remain FP32
- Geometry/timing protocol: B6 d=64 MHA 10355.89 vs SDPA 11130.96 tok/s, 2 warmups/10 reps
- Evidence artifact: `artifacts/titans_stage_b/b6_attention_backend_evidence.json; artifacts/titans_stage_b/a100/b6_attention_backend_evidence.json`
- Seed/timing: seed 20260735; 2 warmups/10 repetitions for CPU and A100 attention timing
- Command: `Python API: run_attention_backend_evidence(); write_attention_backend_evidence()`
- Limitations: A100 Flash rejected the exact additive mask; differentiable training uses the portable math-SDPA kernel.

### exact_accelerated+sdpa

- Paper mapping: Execution substitutions only; nonlinear evolving-gradient recurrence and exact [P,H,S] mask are unchanged
- Output/state/gradient evidence: B3 output/state tensor exact; B6 FP64/FP32 output, state, surprise, input, persistent-token, and attention-gradient parity
- Hardware/dtype/precision: Named NVIDIA A100-SXM4-40GB FP32 with FP32 neural-memory state
- Geometry/timing protocol: A100 two-segment outer training 65.07 tok/s vs 60.65 reference, 1 warmup/3 reps
- Evidence artifact: `artifacts/titans_stage_b/a100/a100_pilot_manifest.json`
- Seed/timing: A100 seeds 20260727/20260735/20260738/20260740; 1 warmup/3 outer-training repetitions
- Command: `seqtrainer-titans-stage-b-a100-pilot --output-dir artifacts/titans_stage_b/a100 --warmup-runs 1 --repetitions 3`
- Limitations: Selected from a small synthetic A100 pilot; Stage C must remeasure representative batching, context, and gradient horizon. Flash remains disabled.

### flash

- Paper mapping: Kernel substitution only; mask may never be weakened
- Output/state/gradient evidence: Forced A100 attempt rejected the exact mask before execution
- Hardware/dtype/precision: Named NVIDIA A100-SXM4-40GB FP16 probe
- Geometry/timing protocol: Unavailable; capability rejection is the result
- Evidence artifact: `artifacts/titans_stage_b/a100/b6_attention_backend_evidence.json`
- Seed/timing: seed 20260735; named-A100 forced exact-mask FP16 Flash probe
- Command: `seqtrainer-titans-stage-b-a100-pilot --output-dir artifacts/titans_stage_b/a100`
- Limitations: forced Flash SDP rejected the exact mask: No available kernel. Aborting execution.
...
A100: **available** — strict A100 pilot verification passed

- Final tests: `/Users/gonzalovidal/opt/anaconda3/envs/seqtrainer/bin/python -m pytest -q` → 94 passed, 7 warnings.
...
- Regenerate audit: `/Users/gonzalovidal/opt/anaconda3/envs/seqtrainer/bin/seqtrainer-titans-stage-b-audit --test-count 94 --warning-count 7`.
...
- a100_pilot: `artifacts/titans_stage_b/a100/a100_pilot_manifest.json`

## Final gate

**READY: Stage C may begin.**
